...
def main():
    ...
    columnTitleRow = "Date, Time, stat_amb, des_amb, state, Amb, Sat"
    ...
    while (1):
        ...
        des_amb_diff = amb - des_temp #if negative want it heat up
        sat_amb_diff = sat_amb_diff/30.0
        print("des_amb: %f sat_amb: %f" %(des_amb_diff, sat_amb_diff))
        # determine new state based on the differences
        if des_amb_diff > 1 and sat_amb_diff > 2:
            new_state = 1
        elif des_amb_diff < -1 and sat_amb_diff > 2:
            new_state = 0
        elif des_amb_diff > 1 and sat_amb_diff < -2:
            new_state = 0
        elif des_amb_diff < -1 and sat_amb_diff < -2:
        ...
        if locked == 0 and new_state != state:
            ...
            print("Move shade")
##            if new_state == 1:
##                control_shade.allUp(shade_state, 0)
##                shade_state = 0
##                
##            else:
...
##                
        ...
        res_ls = [date, cur_time, str(sat_amb_diff), str(des_amb_diff), str(new_state), str(amb), str(sat)]
        res_str = ", ".join(res_ls)
        csv.write(res_str)
        state = new_state
